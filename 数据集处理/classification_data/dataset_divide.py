...
def mkTotalDir(data_path):
    ...
    for i in range(len(dic)):
        current_path = osp.join(data_path, dic[i])
...
def divideTrainValidationTest(source_path, train_path, validation_path, test_path, train_ratio, trainval_ratio):
    """先获取五类图像的名称列表和类别数目"""
    classes_name_list, classes_num = getClassesMes(source_path)
    ...
    for i in range(0, classes_num):
        ...
        train_image_list = source_image_dir[0:int(train_ratio * len(source_image_dir))]
        validation_image_list = source_image_dir[int(train_ratio * len(source_image_dir)):int(trainval_ratio * len(source_image_dir))]
        test_image_list = source_image_dir[int(trainval_ratio * len(source_image_dir)):]
        """
...
if __name__ == '__main__':
    # 不用提前建文件夹，不然会出错
    data_path = './fish_feed/'  # train的上一级
    source_path = './fish_feed/total/'
    train_path = './fish_feed/train/'
    validation_path = './fish_feed/val/'
    test_path = './fish_feed/test/'
    train_ratio = 0.8
    trainval_ratio = 0.9
    mkTotalDir(data_path)
    divideTrainValidationTest(source_path, train_path, validation_path, test_path, train_ratio, trainval_ratio)
