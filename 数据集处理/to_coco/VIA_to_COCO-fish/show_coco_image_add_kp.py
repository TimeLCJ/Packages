...
img_path = './data1/hetun.5.11/imgs'
annFile = './data1/hetun.5.11/coco_kp.json'
...
for i in range(len(imgIds_1)):
    ...
    for ann in anns:
        ...
        if 'keypoints' in ann and type(ann['keypoints']) == list:
            ...
            for sk in sks:
                if np.all(v[sk] > 0):
                    cv2.line(img, tuple(points[sk[0]]), tuple(points[sk[1]]), (255, 0, 0), 5)
            for i in range(len(x)):
                if v[i] > 0:
                    cv2.circle(img, tuple(points[i]), 1, (0, 0, 255), 4)
